find_replace_expr <- function(subvar) {
...
split_formula <- function(formula) {
  ...
  if(length(var_sub) > 0L) {
    ...
    replace_exprs <- lapply(rhs_vars[var_sub_rhs], find_replace_expr)
  }
  else{
    form_no_sub <- formula
    replace_exprs <- NULL
  }
  return(list(form_no_sub = form_no_sub, replace_exprs = replace_exprs))
}
                          ...
                          etastart, control = glmerControl(), ...)
{
  ...
  replace_exprs <- formula_split$replace_exprs
  mc <- match.call()
  ...
  if(length(replace_exprs) == 0L) {
    ...
    modfr_list <- lapply(replace_exprs, parse_replace_expr, data = data,
