...
If `method = "Laplace"` is used, `glmm` automatically tries to check the 
reliability of inference using this approximation, and in this case gives a
warning message that inference the first-order Laplace approximation may be 
unreliable. These checks are experimental, and are currently only used for the
first-order Laplace approximation. An absence of this warning, particularly 
when using other approximation methods, should not be taken to mean that the 
resulting inference will be reliable. These checks can be switched off by 
using `control = list(check_Laplace = FALSE)`.
