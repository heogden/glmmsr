...
```
This gives some estimates of the parameters, but do not know if these
...
We could fit the same model using `glmmsr`, first using the Laplace
...
```
We could try using a more accurate likelihood approximation, such as sequential reduction with `nSL = 2`.
```{r lizards_SR_2}
...
is very different. As we increase `nSL`, the estimate the random
effect standard deviation $\sigma$ increases.
...
which gives an accurate approximation to the likelihood in many cases
for which the Laplace approximation is unreliable. Some cases remain
in which the sequential reduction approximation is too expensive to compute,
despite the poor quality of the Laplace approximation, motivating the need
for other methods for approximating the likelihood. We hope to add such methods
to future versions of `glmmsr`.
