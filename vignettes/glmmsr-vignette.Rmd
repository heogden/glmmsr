...
It is not possible to compute this integral analytically, except in the
...
Here, we fit the model used by @Booth1999, any many other authors.
...
salamanders belongs to. The latent variables $b_i$ and $c_j$ represent
the differeces in propensity to mate between individual animals,
...
It is possible, but time-consuming, to fit the model with the sequential
reduction approximation with $2$ sparse levels, but if we try to use
$3$ sparse levels, we get an error:
...
To ensure the approximation to the likelihood surface is smooth, the samples
used for each value of $\theta$ are constructed by transforming a common
set of standard normal samples.

Since this is a stochastic approximation, we will obtain a different
fit if we run the same code twice. We fit our model for the
`salamander` data twice, each time using importance sampling with $1000$ samples:
```{r salamander_IS_1000_1}
set.seed(1)
mod_sal_IS_1000_1 <- glmm(Mate ~ 0 + Cross + (1 | Male) + (1 | Female),
                          family = binomial, data = salamander,
                          method = "IS", control = list(nIS = 1000),
                          prev_fit = mod_sal_Laplace)
mod_sal_IS_1000_1

mod_sal_IS_1000_2 <- glmm(Mate ~ 0 + Cross + (1 | Male) + (1 | Female),
                          family = binomial, data = salamander,
                          method = "IS", control = list(nIS = 1000),
                          prev_fit = mod_sal_Laplace)
mod_sal_IS_1000_2
```
Both fits agree reasonably well with each other.
...
theta_poss <- cbind(seq(0, 3, by = 0.2), 0.3)
...
mod_sal_equal
```
As before, we can also fit the model with an importance sampling approximation
to the likelihood:
```{r salamander_equal_IS}
mod_sal_equal_IS <- glmm(Mate ~ 0 + Cross + Sub(propen[female_id] + propen[male_id]),
                         propen[sal] ~ 0 + (1 | sal), family = binomial, 
                         data = salamander, method = "IS", 
                         control = list(nIS = 1000), prev_fit = mod_sal_equal)
mod_sal_equal_IS
```
We observe a small increase in the estimate of the random-effects standard
deviation when we use the more accurate approximation to the likelihood.
...
approximation is a very poor approximation, the importance sampling approximation 
