...
approximation. 
 ...
 2. `k`, the level of sparse grid storage. A non-negative integer, where the 
  ...
  which case `glmerSR` passes to `lme4::glmer` to do the fitting.
...
Instead, we can use `glmerSR` to fit the model using the sequential reduction
...
The interface of `glmerSR` also allows fitting
